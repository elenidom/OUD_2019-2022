...
mymeds.z_drugs <- mymeds.pc %>% rename("patid" = "patid_set20") %>% 
  dplyr::select(c(patid, Z_drugs)) %>% 
  distinct() %>%
  filter(Z_drugs > 0) # exclude z_drugss that were prescribed before baseline
...
master.ddi <- left_join(mydata, mymeds.z_drugs, by = "patid")

master.ddi <- master.ddi %>% rename("z_drugs.sta" = "Z_drugs") 

summary(master.ddi)

head(master.ddi)

# keep z_drugs in the appropriate OST interval, exclude presciptions that exceed the fu are have NAs,
# and also exclude prescriptions according to which patients were exposed to z_drugss after the end of fu
master.ddi <- master.ddi %>%
  mutate(
    z_drugs.sta = ifelse(z_drugs.sta >= tstart & z_drugs.sta <= tstop & !is.na(z_drugs.sta), z_drugs.sta, 0)
  )

# arrange data
master.ddi <- master.ddi %>% arrange(patid, tstart, z_drugs.sta) %>% distinct()
...
# exclude patids who were never exposued to Z_drugs - we will append them back later
master.ddi.z_drugsever <- master.ddi %>% group_by(patid) %>% 
  mutate(Z_drugs.ever = ifelse(max(z_drugs.sta) == 0, 1, 0)) %>%
  filter(Z_drugs.ever == 1) %>%
  dplyr::select(-Z_drugs.ever) %>%
  ungroup()

master.ddi.z_drugsever %>% count(patid) # patients with no exposure to Z_drugss

master.ddi.z_drugs <- master.ddi %>% group_by(patid) %>% filter(max(z_drugs.sta) > 0) %>% ungroup()

master.ddi.z_drugs %>% count(patid) # patients with exposure to Z_drugss

master.ddi.z_drugs <- master.ddi.z_drugs %>% dplyr::select(c(id, tstart, tstop, status, z_drugs.sta))
...
#                                   aevent = NA,
#                                   adrug = z_drugs.sta, aedrug = z_drugs.sta + 14, 
#                                   data = master.ddi.z_drugs)
# 
# write.csv(formated.data, "formated.z_drugs.csv")

formated.data <- read.csv("formated.z_drugs.csv")
...
head(mypatids)
master.ddi.z_drugs2 <- left_join(formated.data, mypatids, by = "id")
summary(master.ddi.z_drugs2)


# checks
head(master.ddi.z_drugs2)
sum(master.ddi.z_drugs2$lower < master.ddi.z_drugs2$astart)
sum(master.ddi.z_drugs2$upper > master.ddi.z_drugs2$aend)
master.ddi.z_drugs2 <- master.ddi.z_drugs2 %>% rename("tstart" = "astart", "tstop" = "aend")
master.ddi.z_drugs2 %>% group_by(patid) %>% summarise(N = n_distinct(patid))
...
master.ddi.z_drugs3 <- left_join(master.ddi.z_drugs2, status_epi_data, by = c("patid", "tstart", "tstop"))
summary(master.ddi.z_drugs3)
...
# append NFO with the formated data that contains Z_drugs info per time span
master.ddi.z_drugs3 <- left_join(master.ddi.z_drugs3, NFO_info, by = c("patid", "tstart", "tstop"))

# check the number of NFOs (with the one among patients who did not receive Z_drugs, it is 12,973)
# a <- master.ddi.z_drugs3 %>% 
#   filter(NFO == 1) %>%
#   dplyr::select(c(patid, tstop)) %>%
#   distinct()

# keep NFO in the correct time interval (lower-upper)
master.ddi.z_drugs3 <- master.ddi.z_drugs3 %>% 
  group_by(patid) %>%
  mutate(
...
sum(master.ddi.z_drugs3$NFO_event) # check
...
# 1001  14      29
#
master.ddi.z_drugs3 <- master.ddi.z_drugs3 %>% group_by(patid) %>%
...
master.ddi.z_drugs3 <- master.ddi.z_drugs3 %>% mutate(tstart = tstart2) %>% dplyr::select(-tstart2)

# merge with non-z_drugs data
head(master.ddi.z_drugsever)

master.ddi.z_drugsever <- master.ddi.z_drugsever %>% 
  dplyr::select(c(-index_date, -exit_date, -id, -gender, -epoc)) %>%
  rename("NFO_event" = "NFO")

master.ddi.z_drugs_all <- dplyr::union(master.ddi.z_drugsever, master.ddi.z_drugs3)
master.ddi.z_drugs_all <- master.ddi.z_drugs_all %>% rename("Z_drugs" = "z_drugs.sta")
...
master.ddi.z_drugs_all <- dplyr::left_join(master.ddi.z_drugs_all, testmulti.slice, by = "patid")

head(master.ddi.z_drugs_all)

summary(master.ddi.z_drugs_all)

# ---------------------------------------------------------------------
# NEW VAR: time-varying age that changes at an OST episode
# ---------------------------------------------------------------------

master.ddi.z_drugs_all <- master.ddi.z_drugs_all %>%
  group_by(patid, OST_episode) %>%
  mutate(
    age.tv = max(age + tstop/365.25)
  ) %>% ungroup()

summary(master.ddi.z_drugs_all$age.tv)
head(master.ddi.z_drugs_all)
...
summary(master.ddi.z_drugs_all$age.id)
...
master.ddi.z_drugs_all <- left_join(master.ddi.z_drugs_all, modality, by = "patid")
...
master.ddi.z_drugs_all <- master.ddi.z_drugs_all %>% filter(tstart != tstop)


# ----------------------------------------------------------------------------------------
# All the data (z_drugs on/off and NFO yes/no) -*** CRUDE RATES / RELATIVE ESTIMATES ***-
# ----------------------------------------------------------------------------------------

# summary of OAT episode duration when on Z_drugss
a <- master.ddi.z_drugs_all %>% 
  group_by(patid) %>% 
  filter(Z_drugs == 1) %>%
  mutate(Z_drugs.fu = sum(tstop - tstart)) %>%
  ungroup() 

a1 <- a %>% group_by(patid) %>% slice(1) %>% ungroup()

hist(a1$Z_drugs.fu, breaks = 100, main = "Observation years among patients who received Z_drugs",
     xlab = "time (years)",
     xlim = c(0, 7000))

summary(a1$Z_drugs.fu)
...
master.ddi.z_drugs_all <- master.ddi.z_drugs_all %>% 
  group_by(patid) %>%
  mutate(Z_drugs = ifelse(Z_drugs == 0 & tstop - tstart <= 7 & # DEFAULT: 7 / SENS: 14
                         lag(Z_drugs == 1, default = first(Z_drugs) == 1) & 
                         lead(Z_drugs == 1, default = last(Z_drugs) == 1), 
                       1, Z_drugs)
  ) %>% ungroup()
...
# -------------------------------------------------------------------
# on/off Z_drugss any time OST (Crude estimates and regression models)
# -------------------------------------------------------------------

master.ddi.z_drugs_all %>%
...
poi_data <- master.ddi.z_drugs_all %>% 
  dplyr::select(c("patid", "tstart", "tstop", "Z_drugs", "gender", "age.id", "epoc",
...
# on/off Z_drugs
poi_data <- poi_data %>%
  group_by(patid, Z_drugs) %>%
  mutate(
    fu = sum(diff), # fu on/off Z_drugs
    NFO_count = sum(NFO_event)
  ) %>% ungroup()

# keep only some vars to generate models
poi_data <- poi_data %>% 
  dplyr::select(c(patid, Z_drugs, gender, age.id, epoc, epi_n, 
...
poi_model.0 <- glm(NFO_count ~ factor(Z_drugs) + offset(log(fu)),
...
# NB
nb_model.0 <- glm.nb(NFO_count ~ factor(Z_drugs) + offset(log(fu)), data = poi_data, subset = fu > 0)
...
poi_model.1 <- glm(NFO_count ~ factor(Z_drugs) + offset(log(fu)) + factor(epoc) + age.id +
...
# NB
nb_model.1 <- glm.nb(NFO_count ~ factor(Z_drugs) + offset(log(fu)) + factor(epoc) + age.id +# epi_n +
...
# poisson
poi_model.1.w <- glm(NFO_count ~ factor(Z_drugs) + offset(log(fu)) + factor(epoc),# + epi_n,
                     data = poi_data, subset = fu > 0,
...
# NB
nb_model.1.w <- glm.nb(NFO_count ~ factor(Z_drugs) + offset(log(fu)) + factor(epoc) + epi_n,
                       data = poi_data, subset = fu > 0,
                       weights = mysipws)
...
# -------------------------- end of any time OST on/off Z_drugss results -----------------------

# <|>

# -----------------------------------------------------------------------------------------
# in/out OST on/off Z_drugss
# -----------------------------------------------------------------------------------------

master.ddi.z_drugs_all %>%
  group_by(status, Z_drugs) %>%
  summarise(PY = sum(tstop - tstart)/365.25,
            Events = sum(NFO_event),
            RR = Events/PY * 100)

# ON/OFF Z_drugss in OST
poi_data <- master.ddi.z_drugs_all %>% mutate(diff = tstop - tstart) %>% filter(status == "in")
...
# NB
nb_model.0 <- glm.nb(NFO_count ~ factor(Z_drugs) + offset(log(fu)),
                     data = poi_data, subset = fu > 0)
...
# adjusted model
poi_model.1 <- glm(NFO_count ~ factor(Z_drugs) + offset(log(fu)) + factor(therapy) + epi_n +
...
# NB
nb_model.1 <- glm.nb(NFO_count ~ factor(Z_drugs) + offset(log(fu)) + factor(therapy) +# epi_n +
...
# weighted model
poi_model.1.w <- glm(NFO_count ~ factor(Z_drugs) + offset(log(fu)),
...
# -------------------------- in/out OST on Z_drugs results --------------------------------

# <|>

# -----------------------------------------------------------------------------------------
# ON/OFF Z_drugss out of OST
# -----------------------------------------------------------------------------------------

poi_data <- master.ddi.z_drugs_all %>% mutate(diff = tstop - tstart) %>% filter(status == "out")
...
# adjusted
poi_model.1 <- glm(NFO_count ~ factor(Z_drugs) + offset(log(fu)) +# age.id + epi_n +
...
# adjusted NB
nb_model.1 <- glm.nb(NFO_count ~ factor(Z_drugs) + offset(log(fu)) + factor(therapy) + 
...
# weighted
poi_model.1w <- glm(NFO_count ~ factor(Z_drugs) + offset(log(fu)) + factor(epoc),
                    data = poi_data, subset = fu > 0, family = poisson, weights = mysipws)
...
# weighted NB
nb_model.1w <- glm.nb(NFO_count ~ factor(Z_drugs) + offset(log(fu)) + factor(epoc),# + epi_n,
                      data = poi_data, subset = fu > 0, weights = mysipws)
...
# -------------------------- end of on/off Z_drugs out of OST results -------------------------
...
poi_data <- master.ddi.z_drugs_all %>% mutate(diff = tstop - tstart)

# ==> METHADONE
poi_data.inmeth <- poi_data %>% filter(therapy == 1 & status == "in") # in methadone only

poi_data.inmeth <- poi_data.inmeth %>% 
  dplyr::select(c(patid, Z_drugs, gender, age.id, epoc, epi_n, mysipws, diff, NFO_event))
...
# adjusted
poi_model.inm.adj <- glm(NFO_count ~ factor(Z_drugs) + offset(log(fu)) + age.id + epi_n +
...
# weighted
poi_model.inm.w <- glm(NFO_count ~ factor(Z_drugs) + offset(log(fu)) + factor(epoc) + epi_n,
...
# adjusted
nb_model.inm.adj <- glm.nb(NFO_count ~ factor(Z_drugs) + offset(log(fu)) + age.id + factor(epoc) +
...
# weighted
nb_model.inm.w <- glm.nb(NFO_count ~ factor(Z_drugs) + offset(log(fu)) + factor(epoc),
...
# adjusted
poi_model.inb.adj <- glm(NFO_count ~ factor(Z_drugs) + offset(log(fu)) +# age.id + factor(epoc) +
                            factor(gender) + epi_n,
                         data = poi_data.inbup, subset = fu > 0, family = poisson)
...
# weighted
poi_model.inb.w <- glm(NFO_count ~ factor(Z_drugs) + offset(log(fu)),# + factor(epoc) + epi_n,
...
# unadjusted
nb_model.inb.unadj <- glm.nb(NFO_count ~ factor(Z_drugs) + offset(log(fu)),
...
nb_model.inb.adj <- glm.nb(NFO_count ~ factor(Z_drugs) + offset(log(fu)) + factor(epoc),
                            #+ age.id + factor(gender),
                           data = poi_data.inbup, subset = fu > 0)
...
# weighted
nb_model.inb.w <- glm.nb(NFO_count ~ factor(Z_drugs) + offset(log(fu)) + factor(epoc),
