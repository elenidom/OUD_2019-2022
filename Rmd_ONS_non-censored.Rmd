...
# filter patients who changed medication within the same episode and those with registration < 6 months at index
# test <- test %>% filter(switcher_epi == 0 & prereg >= 6)

# cohort size
#test %>% summarise(Np = n_distinct(patid)) 
...
library(ggplot2)
                       #       ---------------------------------------------------
database <- "Combined" # <--- | USER INPUT to populate plots with correct titles  |
                       #       ---------------------------------------------------
...
# Separate the other category to classify these causes of deaths
# other_cause <- all_cause 
# other_cause <- data.table(other_cause)
# other_cause <- other_cause[death_cat == "Other *** ", .(patid, cause)]
# other_cause <- other_cause %>% distinct()
